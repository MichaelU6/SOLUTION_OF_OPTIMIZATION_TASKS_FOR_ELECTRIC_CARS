...
class Herusitic:
    ...
    def run_heuristic(self):
        ...
        customerCopy = list(range(1, self.EVRP.NUM_OF_CUSTOMERS + 1))
        # Vsetci zakaznici idu
        for vehicle_id in range(self.num_vehicles):
            newCustomerCopy = self.construct_solution(vehicle_id, customerCopy)
            customerCopy = newCustomerCopy
            #print(customerCopy)
    ...
    def construct_solution(self, vehicle_id, customer_group):
        ...
        while i == 1:
            ...
            if to_node is False:
                #print(from_node, energy_temp, vehicle_id)
            ...
            elif (capacity_temp + self.EVRP.get_customer_demand(to_node ) <= self.EVRP.MAX_CAPACITY and
        ...
        if self.solution['tour'][vehicle_id][self.solution['steps'][vehicle_id]] != self.EVRP.DEPOT:
        ...
        #    self.best_sol['steps'][vehicle_id] += 1
            ...
            self.find_path_in_depo(self.solution['tour'][vehicle_id][self.solution['steps'][vehicle_id]], energy_temp, vehicle_id)    
        #print(self.EVRP.MAX_CAPACITY)
        #skontrolujem riesenie pre vozidlo ci je dokoncene a ci obsluzil vsektych zakaznikov
        print("cesta ",self.solution['tour'][vehicle_id])
